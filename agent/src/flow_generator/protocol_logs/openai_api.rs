...
pub const METRIC_TTFT_MS: &str = "llm_ttft_ms";
pub const METRIC_TPOT_MS_EST: &str = "llm_tpot_ms_est";
...
pub const METRIC_TOTAL_STREAM_MS: &str = "llm_total_stream_ms";
...
impl OpenAISession {
    ...
    /// TPOT = response latency / output_tokens (throughput estimate assuming
    /// the model generated tokens at a constant rate during inference).
    pub fn compute_timings(&self) -> (Option<f64>, Option<f64>) {
        if self.is_stream {
            let ttft_ms = self.first_output_ts_us.map(|first| {
                let diff = first.saturating_sub(self.request_ts_us);
                diff as f64 / 1000.0
            });

            let tpot_ms_est = match (
                ...
                (Some(first), Some(last), Some(usage)) if usage.output_tokens > 0 => {
                    ...
                    Some(span_us as f64 / divisor as f64 / 1000.0)
                }
                _ => None,
            };
            ...
            let ttft_ms = end_ts.saturating_sub(self.request_ts_us) as f64 / 1000.0;
            let tpot_ms_est = self.usage.as_ref().and_then(|u| {
                if u.output_tokens > 0 {
                    Some(ttft_ms / u.output_tokens as f64)
                } else {
                    None
                }
            });
            (Some(ttft_ms), tpot_ms_est)
        }
    }
    ...
    pub fn populate_log(&self, attrs: &mut Vec<KeyVal>, metrics: &mut Vec<MetricKeyVal>) {
        ...
        let (ttft_ms, tpot_ms_est) = self.compute_timings();
        if let Some(v) = ttft_ms {
            push_metric(metrics, METRIC_TTFT_MS, v);
        }
        if let Some(v) = tpot_ms_est {
            push_metric(metrics, METRIC_TPOT_MS_EST, v);
        }
        ...
        // Total request-to-completion duration.
        // For streaming: request → final [DONE] event (full stream wall-clock time).
        // For non-streaming: request → response body received (same as llm_ttft_ms).
        // The standard `rrt` field holds first-response latency (packet-level);
        // this metric captures end-to-end LLM inference cost in both modes.
        if let Some(end_ts) = self.stream_end_ts_us {
            let total_ms = end_ts.saturating_sub(self.request_ts_us) as f64 / 1000.0;
            push_metric(metrics, METRIC_TOTAL_STREAM_MS, total_ms);
        }
    }
}
...
mod tests {
    ...
    fn test_chat_completions_sse_ttft_tpot() {
        ...
        // ttft = (100_000 - 0) / 1000 = 100ms
        assert!((ttft.unwrap() - 100.0).abs() < 1.0);
        // tpot = (500_000 - 100_000) / max(5-1, 1) / 1000 = 400_000 / 4 / 1000 = 100ms
        assert!((tpot.unwrap() - 100.0).abs() < 1.0);
    }
    ...
    fn test_non_streaming_timings() {
        // Non-streaming: request at t=0, response at t=2000ms.
        // TTFT = TOTAL = 2000ms, TPOT = 2000 / 10 output_tokens = 200ms.
        ...
        let (ttft, tpot) = s.compute_timings();
        assert!(
            (ttft.unwrap() - 2000.0).abs() < 1.0,
            "ttft should be 2000ms, got {:?}",
            ttft
        );
        assert!(
            (tpot.unwrap() - 200.0).abs() < 1.0,
            "tpot should be 200ms, got {:?}",
            tpot
        );
        ...
        assert!(metric_map.contains_key("llm_ttft_ms"), "ttft missing");
        assert!(metric_map.contains_key("llm_tpot_ms_est"), "tpot missing");
        assert!(
            metric_map.contains_key("llm_total_stream_ms"),
            "total_stream_ms missing"
        );
        assert!((metric_map["llm_ttft_ms"] - 2000.0).abs() < 1.0);
        assert!((metric_map["llm_tpot_ms_est"] - 200.0).abs() < 1.0);
        assert!((metric_map["llm_total_stream_ms"] - 2000.0).abs() < 1.0);
    }
