...
mod tests {
    ...
    fn test_openai_metrics_in_send_log() {
        ...
        // Total stream duration: 5_000_000 µs - 0 µs = 5_000_000 µs = 5000 ms.
        let total_ms = metric_map
            .get("llm_total_stream_ms")
            .copied()
            .expect("llm_total_stream_ms missing");
        assert!(
            (total_ms - 5000.0).abs() < 1.0,
            "expected ~5000 ms total stream, got {total_ms}"
        );
    }
}
